...
export default async function EditProjectPage({ params }: EditProjectPageProps) {
  ...
  const { data: project, error } = await supabase
    .from('projects')
    .select('id, business_name, business_type, location, project_briefs(business_story, primary_goal, website_copy, important_links, tone, color_preference), design_references(id, url, notes)')
    .eq('id', projectId)
    .single();
