...
export default async function ProjectPreviewPage({params,searchParams}:{params:Promise<{projectId:string}>;searchParams:Promise<{version?:string}>}) {
  ...
  return <div><header className="preview-toolbar"><span>{version.visibility==='preview'?'הצעת שינוי פרטית':'תצוגה פרטית'} · גרסה {version.version_number}</span><Link href={`/dashboard/projects/${projectId}`}>חזרה לפרויקט</Link></header><PreviewViewport>{getCuratedDemo(version.content)?<CuratedDemo content={version.content} />:<SiteRenderer plan={version.content} projectId={projectId} versionId={version.id} />}</PreviewViewport></div>;
}
