...
export default async function ProjectPage({params}:{params:Promise<{projectId:string}>}){
  ...
  const {data:project}=await client.from('projects').select('id,business_name,business_type,location,status,project_briefs(business_story,primary_goal,website_copy,tone),project_assets(id,original_name,mime_type,size_bytes)').eq('id',projectId).single();
  if(!project)notFound();
  ...
  return <main className="app-shell">
    ...
    {live ? <LeadInbox projectId={projectId}/> : null}
    <DeleteProjectButton projectId={projectId} businessName={project.business_name} isPublished={Boolean(live)} isDemo={Boolean(demo)}/>
  </main>;
}
