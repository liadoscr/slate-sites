...
export function errorCategory(error: unknown) {
  const value = error && typeof error === 'object' ? error as { message?: unknown; status?: unknown; cause?: { code?: unknown } } : {};
  const message = typeof value.message === 'string' ? value.message : '';
  const status = typeof value.status === 'number' && value.status >= 400 && value.status <= 599 ? value.status : undefined;
  const category = status === 429 || /RESOURCE_EXHAUSTED|quota|rate limit/i.test(message) ? 'provider_quota'
...
export function logOperationError(error: unknown, context: Context): string {
  ...
  const event = {
    ...
    phase: ['preparing', 'designing', 'saving'].includes(context.phase ?? '') ? context.phase : undefined,
    ...errorCategory(error),
  };
