...
The three curated full-page demos also use the shared scroll controller: ORANGE.GEL and FORMA use subtle motion, MOVE uses expressive motion. Below-hero sections reveal once; heroes, layouts and content are unchanged. Their shared motion stylesheet adds small CTA-arrow feedback and reduced-motion/focus safeguards, including suppression of the nail demo's existing ticker/hover animation under reduced motion. The compact homepage carousel is unchanged. These hardcoded demo enhancements are renderer updates; they do not require customer-data edits or a database migration.

Scroll reveals run once per page/version view, with modest hover feedback. There is no autoplay loop, parallax or scroll hijacking. The hero stays visible, and content remains readable without JavaScript or supported animation APIs. The renderer respects reduced-motion preferences, including changes while the page is open, and cancels reveals when keyboard focus enters the content.
