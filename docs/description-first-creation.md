...
Gemini chooses the supported layout, text, and a stock subject enum in one model call. The default starter does not override an automatic layout. A server-owned category map converts the enum into generic Pexels search terms; private descriptions, names and contact details are never sent to Pexels. An unknown category produces a usable photo-free draft, not invented search URLs.
...
- Stock selection/search failures are logged with phase `sourcing`. The text draft is saved with a review note; users can upload their own photos without regenerating.
